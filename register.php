...
if(isset($_POST['send']))
{
	...
	if ((strlen($username) < 3 || (strlen($username) > 20)))  //strlen długość stringa
	{
		$valid = false;
		$answer = "Twój login musi mieć od 3 do 20 znaków.";
	}

	if (ctype_alnum($username)==false)    //Checks if all of the characters in the provided string, text, are alphanumeric.
	{
		$valid = false;
	...
	$email = $_POST['email'];
	$emailB = filter_var($email, FILTER_SANITIZE_EMAIL); //Filters a variable ($email) with a specified filter in this case the filter is FILTER_SANITIZE_EMAIL
